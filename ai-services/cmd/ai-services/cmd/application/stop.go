...
func stopApplication(cmd *cobra.Command, client *podman.PodmanClient, appName string, podnames []string) error {
	...
	*/
	...
	if len(podnames) > 0 {
		...
		if len(podsToStop) == 0 {
			klog.Infof("No valid pods found to stop for application: %s\n", appName)
			return nil
		}
	} else {
	...
	klog.Infoln("Below pods will be stopped:")
	for _, pod := range podsToStop {
		klog.Infof("\t-> %s\n", pod.Name)
	}

	klog.Infof("Are you sure you want to stop above pods? (y/N): ")
	...
	if !confirmStop {
		klog.Infof("Skipping stopping of pods\n")
		return nil
	}

	klog.Infof("Proceeding to stop pods...\n")

	// 3. Proceed to stop only the valid pods
	var errors []string
	for _, pod := range podsToStop {
		klog.Infof("Stopping the pod: %s\n", pod.Name)
		if err := client.StopPod(pod.Id); err != nil {
			errMsg := fmt.Sprintf("%s: %v", pod.Name, err)
			errors = append(errors, errMsg)
			continue
		}
		klog.Infof("Successfully stopped the pod: %s\n", pod.Name)
	}
