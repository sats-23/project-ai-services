...
Available checks to skip:
  ...
  aiservices bootstrap validate --verbose`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			klog.Infof("Running bootstrap validation...")

			skip := helpers.ParseSkipChecks(skipChecks)
			if len(skip) > 0 {
				klog.Warningf("WARNING: Skipping validation checks", skipChecks)
			}
			...
			klog.Infof("All validations passed")
			return nil
		},
	}
...
func rootCheck() error {
	...
	if euid == 0 {
		klog.Infof("Current user is root.")
	} else {
...
func validateOS() error {
	...
	klog.Infof("Operating system is RHEL", zap.String("version", version))
	return nil
}
...
func validateRHNRegistration() error {
	...
	klog.Infof("System is registered with RHN")
	return nil
}
...
func validatePowerVersion() error {
	...
	if err == nil && strings.Contains(strings.ToLower(string(data)), "power11") {
		klog.Infof("System is running on IBM Power11 architecture")
		return nil
	}
...
func validateSpyreAttachment() error {
	...
	klog.Infof("IBM Spyre Accelerator is attached to the LPAR")
	return nil
}
...
func validateNumaNode() error {
	...
	klog.Infof("NUMA node alignment on LPAR: 1")
	return nil
}
